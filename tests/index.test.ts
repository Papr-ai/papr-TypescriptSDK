...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new Papr({ logger: logger, logLevel: 'debug', xAPIKey: 'My X API Key' });
    ...
    test('default logLevel is warn', async () => {
      const client = new Papr({ xAPIKey: 'My X API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new Papr({ logger: logger, logLevel: 'info', xAPIKey: 'My X API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      const client = new Papr({ logger: logger, xAPIKey: 'My X API Key' });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['PAPR_LOG'] = 'debug';
      const client = new Papr({ logger: logger, logLevel: 'off', xAPIKey: 'My X API Key' });
  ...
  test('normalized method', async () => {
    ...
    const client = new Papr({ baseURL: 'http://localhost:5000/', xAPIKey: 'My X API Key', fetch: testFetch });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Papr({ baseURL: 'http://localhost:5000/custom/path/', xAPIKey: 'My X API Key' });
    ...
    test('explicit option', () => {
      const client = new Papr({ baseURL: 'https://example.com', xAPIKey: 'My X API Key' });
    ...
    test('in request options overridden by client options', () => {
      const client = new Papr({ xAPIKey: 'My X API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['PAPR_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Papr({ xAPIKey: 'My X API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Papr({ maxRetries: 4, xAPIKey: 'My X API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Papr({ xAPIKey: 'My X API Key' });
    expect(client2.maxRetries).toEqual(2);
  });

  describe('withOptions', () => {
    test('creates a new client with overridden options', () => {
      const client = new Papr({ baseURL: 'http://localhost:5000/', maxRetries: 3, xAPIKey: 'My X API Key' });
    ...
    test('respects runtime property changes when creating new client', () => {
      const client = new Papr({ baseURL: 'http://localhost:5000/', timeout: 1000, xAPIKey: 'My X API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['PAPR_MEMORY_API_KEY'] = 'My X API Key';
    const client = new Papr();
    expect(client.xAPIKey).toBe('My X API Key');
  });

  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['PAPR_MEMORY_API_KEY'] = 'another My X API Key';
    const client = new Papr({ xAPIKey: 'My X API Key' });
    expect(client.xAPIKey).toBe('My X API Key');
  });
});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new Papr({ xAPIKey: 'My X API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new Papr({ xAPIKey: 'My X API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Papr({ xAPIKey: 'My X API Key', fetch: testFetch });
